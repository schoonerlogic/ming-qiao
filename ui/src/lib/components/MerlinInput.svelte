<script lang="ts">
    ...
    let { threadId, sender = "merlin" }: Props = $props();
    ...
    async function handleSubmit() {
        ...
        try {
            ...
            console.log('[MerlinInput] Message injected successfully as', sender);
...
<div class="merlin-input bg-white border border-gray-200 rounded-lg p-4">
    <div class="flex items-start gap-2 mb-3">
        <div
            class="w-8 h-8 rounded-full bg-purple-500 flex items-center justify-center text-white font-semibold flex-shrink-0"
        >
            M
        </div>
        <div class="flex-1">
    ...
    <div class="space-y-3">
        ...
        <div class="flex items-center gap-2">
            ...
            >
                ...
                <svg
                    class="w-4 h-4 ml-1 {showActions
                        ? 'rotate-180'
            ...
            {#if showActions}
                <div class="flex gap-1 flex-wrap">
                    {#each actions as act}
                        <button
                            class="px-3 py-1 text-sm rounded-md border {action ===
                            act
