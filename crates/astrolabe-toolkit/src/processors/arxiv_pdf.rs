...
//!
//! Note: PDF text extraction requires an external tool. This processor
//! decodes the base64 attachment and provides the raw bytes. Full PDF
//! parsing can be added when a pure-Rust PDF library is integrated.
//!
//! Agent: luban
...
use crate::processors::arxiv_url::{
    category_tag, extract_arxiv_id, fetch_arxiv_metadata_blocking, format_authors,
};
...
impl ArtifactProcessor for ArxivPdfProcessor {
    ...
    fn process(&self, artifact: &RawArtifact) -> Result<ProcessorResult, ProcessorError> {
        ...
        // Build content from API metadata (PDF text extraction deferred)
