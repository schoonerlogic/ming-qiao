...
```bash
curl -s http://localhost:3000/api/inbox/luban | jq .
```
...
```bash
curl -s http://localhost:3000/api/inbox/aleph | jq .
```
...
```bash
curl -s -X POST http://localhost:3000/api/thread/THREAD_ID_HERE/reply \
  -H "Content-Type: application/json" \
  -d '{
    "from": "aleph",
